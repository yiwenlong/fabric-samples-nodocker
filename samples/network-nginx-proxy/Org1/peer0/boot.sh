...
echo "[program:FABRIC-NODOCKER-Org1-peer0]" > "$dst_file"
echo "command=$BOOT_DIR/peer node start" >> "$dst_file"
echo "directory=$BOOT_DIR" >> "$dst_file"
echo "redirect_stderr=true" >> "$dst_file"
echo "stdout_logfile=$BOOT_DIR/FABRIC-NODOCKER-Org1-peer0.log" >> "$dst_file"
echo "Supervisor config file generate:" "$dst_file"

supervisorctl update
echo Staring: "FABRIC-NODOCKER-Org1-peer0"
sleep 3
supervisorctl status | grep "FABRIC-NODOCKER-Org1-peer0"
